...
const Navbar = ({
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                const res = await axios.get(`${API_BASE_URL}/api/products/autocomplete?q=${encodeURIComponent(searchQuery.trim())}`);
    ...
    return (
        <nav className="navbar navbar-expand-lg sticky-top custom-navbar navbar-dark">
            <div className="container">
                {/* Brand Logo */}
                <a className="navbar-brand d-flex align-items-center gap-2 fw-bold text-gradient fs-4" href="#" onClick={(e) => { e.preventDefault(); onSelectCategory(null); }}>
                ...
                <div className="collapse navbar-collapse" id="navbarContent">
                    ...
                    <div className="dropdown me-lg-3 my-2 my-lg-0">
                        <button className="btn btn-outline-light dropdown-toggle d-flex align-items-center gap-1" type="button" data-bs-toggle="dropdown">
                            <i className="bi bi-grid-fill me-1"></i> Categories
                        </button>
                        <ul className="dropdown-menu dropdown-menu-dark shadow-lg">
                            <li>
                                <button className="dropdown-item" onClick={() => onSelectCategory(null)}>
                                    All Categories
                                </button>
                            </li>
                            ...
                            {categories.map(cat => (
                                <li key={cat.id}>
                                    <button className="dropdown-item" onClick={() => onSelectCategory(cat.id)}>
                                        {cat.name}
                                    </button>
                                </li>
                            ))}
                    ...
                    <div className="flex-grow-1 position-relative me-lg-3 my-2 my-lg-0" ref={searchRef}>
                        ...
                        {showSuggestions && suggestions.length > 0 && (
                            <div className="autocomplete-menu shadow-lg rounded mt-1 bg-dark border border-secondary text-light">
                                ...
                                {suggestions.map(p => (
                                    ...
                                    >
                                        <img src={p.imageUrl} alt={p.name} className="rounded object-fit-cover" width="45" height="45" />
                                        <div className="flex-grow-1 overflow-hidden">
                                            <div className="fw-semibold text-truncate text-light">{p.name}</div>
                                            <div className="small text-muted d-flex align-items-center gap-2">
                                                <span className="badge bg-secondary">{p.category?.name}</span>
                                                {p.brand && <span>{p.brand}</span>}
                                            </div>
                                        </div>
                                        <div className="fw-bold text-success">${p.price?.toFixed(2)}</div>
                                    </div>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="d-flex align-items-center gap-3 ms-auto">
                        {/* Cart Button */}
                        <button className="btn btn-outline-light position-relative d-flex align-items-center gap-2 px-3 py-2 rounded-pill" onClick={onOpenCart}>
                        ...
                        {user ? (
                            <div className="dropdown position-relative">
                                ...
                                {showUserDropdown && (
                                    <div className="dropdown-menu dropdown-menu-end dropdown-menu-dark show shadow-lg mt-2 border-secondary position-absolute">
                                        ...
                                        <button className="dropdown-item py-2 d-flex align-items-center gap-2" onClick={() => { setShowUserDropdown(false); onOpenProfile('info'); }}>
                                            <i className="bi bi-person-circle"></i> Profile Dashboard
                                        </button>
                                        <button className="dropdown-item py-2 d-flex align-items-center gap-2" onClick={() => { setShowUserDropdown(false); onOpenProfile('orders'); }}>
                                            <i className="bi bi-box-seam"></i> Order History
                                        </button>
                                        <button className="dropdown-item py-2 d-flex align-items-center gap-2" onClick={() => { setShowUserDropdown(false); onOpenProfile('addresses'); }}>
                                            <i className="bi bi-geo-alt"></i> Saved Addresses
                                        </button>
                                        <div className="dropdown-divider"></div>
                                        <button className="dropdown-item py-2 text-danger d-flex align-items-center gap-2" onClick={() => { setShowUserDropdown(false); logout(); }}>
                            ...
                            <button className="btn btn-gradient-primary rounded-pill px-4 py-2 fw-semibold" onClick={() => onOpenAuth('login')}>
